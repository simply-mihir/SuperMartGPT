...
export default function ChatProductCard({ product, language = "english" }: ChatProductCardProps) {
  ...
  return (
    <div className="p-3 border border-border/40 rounded-xl bg-white hover:shadow-md transition-all duration-200">
      ...
      {product.description && (
        <div className="text-xs text-muted-foreground line-clamp-2 mt-1">{product.description}</div>
      )}
      <div className="text-primary font-bold text-base mt-2">₹{priceValue.toFixed(2)}</div>
      <div className="mt-3">
        <button
